...
def get_collect_file_name(utc_now: datetime, chunk_count: int, chunk_no: int) -> str:
    ...
    if chunk_count and chunk_count > 1:
        chunk_prefix = f'chunk{chunk_no}of{chunk_count}'
    result = f'{chunk_prefix}_{settings.FILE_NAME_COLLECT_PREFIX}{format_file_timestamp(utc_now)}{settings.FILE_NAME_COLLECT_SUFFIX}'
    return result
